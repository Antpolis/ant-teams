...
run_preflight() {
  ...
  # ERR-1.1 item 3 / OBS-3.2: node ≥18 on PATH (retained for inspect_repo.js
  # and AGENTS.md generation; SPEC-004-T3 will remove this requirement).
  # Note: ensure_opencode_config and ensure_project_runtime_env no longer
  # require node — they use jq for strict JSON validation and env computation.
...
# Default-only AGENTS.md generator (v0.4.0). Produces a minimal baseline the
# founder customizes through conversation. No inspection, no prompts.
generate_agents_md_content() {
  ...
  if [[ -f "$project_dir/.github-project.env" ]]; then
    vault_path="$(grep "^export ANT_TEAM_DOCS_VAULT_PATH=" "$project_dir/.github-project.env" 2>/dev/null | sed "s/^export ANT_TEAM_DOCS_VAULT_PATH='\(.*\)'/\1/" || true)"
    project_path="$(grep "^export ANT_TEAM_DOCS_PROJECT_PATH=" "$project_dir/.github-project.env" 2>/dev/null | sed "s/^export ANT_TEAM_DOCS_PROJECT_PATH='\(.*\)'/\1/" || true)"
    gh_owner="$(grep "^export ANT_TEAM_GITHUB_OWNER=" "$project_dir/.github-project.env" 2>/dev/null | sed "s/^export ANT_TEAM_GITHUB_OWNER='\(.*\)'/\1/" || true)"
    gh_project="$(grep "^export ANT_TEAM_GITHUB_PROJECT_NUMBER=" "$project_dir/.github-project.env" 2>/dev/null | sed "s/^export ANT_TEAM_GITHUB_PROJECT_NUMBER='\(.*\)'/\1/" || true)"
...
EOF
  ...
  printf '## Local Configuration Files\n\n'
  printf '- `AGENTS.md` — This file — canonical agent guidance for this repository\n'
  [[ -f "$project_dir/.github-project.env" ]] && printf '- `.github-project.env` — ANT_TEAM_* runtime exports — the sole project config source; source it for GitHub, documentation, and worktree metadata\n'
  ...
  [[ -n "$oc_path" ]] && printf '- `%s` — OpenCode runtime config\n' "$oc_path"
  printf '\n'
}
