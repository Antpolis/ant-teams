...
Use role-memory only when completion reveals a new durable, reusable lesson after GitHub collaboration is updated.
...
- If implementation changed durable product intent, architecture, governance, a runbook, or a reusable lesson, the appropriate Obsidian note was updated with wikilinks and linked from the issue or PR.
...
- The GitHub issue and PR contain builder handoff, reviewer findings, review-loop context, the final closing message, verification result, and approval state.
- Any role-memory update is limited to a new durable, reusable lesson; no per-task or no-op memory update is required.

## Required Review Steps

1. Read the GitHub issue first, including scope, non-goals, acceptance criteria, verification, and `Durable Context`.
2. Open the canonical SPEC and every applicable ARCH, ADR, GOV, and runbook URL from the issue's Durable Context.
3. Read the linked PR and review discussion.
4. Read the GitHub issue and PR status, handoff, review-loop, and final-closure records.
5. If a required durable-context URL is missing, ambiguous, stale, or conflicts with the issue or PR, do not approve completion; record the gap in GitHub and route it to tech-lead.
6. Confirm documentation changes were made in the resolved Obsidian project folder when the task requires documentation changes, and that the issue or PR links to the updated durable note.
7. Inspect changed files.
8. Compare implementation against scope, definition of done, and acceptance tests.
9. Run or verify evidence for required commands.
10. Confirm the implementation branch has not been merged back without reviewer approval.
11. Append review, verification, approval, blocker, or defer-task notes to the GitHub issue or PR.
12. Use role-memory only if a new durable, reusable lesson should be captured; otherwise make no memory update.
13. Update the GitHub issue state and completion notes if editing is allowed.
...
- Keep each loop's findings, fixes, and decisions in the GitHub issue or PR. Update role memory only when a loop reveals a new durable, reusable lesson.
...
- [ ] Docs/config updated if needed and linked from GitHub
...
- [ ] Any new durable role lesson captured (if applicable)
...
- Review loop count
- Role memory lesson captured (if applicable)
- Evidence used
- Verification result
...
- Treat GitHub issue state, GitHub Project Workflow State, issue and PR comments, final decision and closure comments, and PR code-review results as the authoritative operational collaboration record.
- Do not close or validate work based on local task-file state alone.
- Use the central Obsidian project folder only for curated durable specs, architecture, ADR, governance, runbook, and reusable knowledge that GitHub issues and milestones link back to.
