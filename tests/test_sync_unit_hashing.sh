...
mkdir -p "$FIX/.opencode/skills/hasher"
printf -- '---\nname: hasher\ndescription: h\n---\n\nbody\n' > "$FIX/.opencode/skills/hasher/SKILL.md"
: > "$FIX/.opencode/skills/hasher/empty.txt"            # zero-byte file
printf 'abc' > "$FIX/.opencode/skills/hasher/known.txt" # known vector (no newline)
# Binary content (non-UTF8 bytes incl. NULs).
printf '\x00\x01\x02\xff\xfe\n' > "$FIX/.opencode/skills/hasher/bin.dat"
...
EMPTY_GOT="$(sync_sha256 "$FIX/.opencode/skills/hasher/empty.txt")"
ABC_GOT="$(sync_sha256 "$FIX/.opencode/skills/hasher/known.txt")"
BIN_GOT="$(sync_sha256 "$FIX/.opencode/skills/hasher/bin.dat")"
...
assert_eq "manifest empty.txt hash == source" \
  "$(sync_manifest_file_hash "$MANIFEST" hasher ".opencode/skills/hasher/empty.txt")" "$EMPTY_GOT"
assert_eq "manifest known.txt hash == source" \
  "$(sync_manifest_file_hash "$MANIFEST" hasher ".opencode/skills/hasher/known.txt")" "$ABC_GOT"
assert_eq "manifest bin.dat hash == source" \
  "$(sync_manifest_file_hash "$MANIFEST" hasher ".opencode/skills/hasher/bin.dat")" "$BIN_GOT"

# --- DM-2: hash pattern ^[a-f0-9]{64}$ (no uppercase, no short) --------------
SKILL_MD_HASH="$(sync_manifest_file_hash "$MANIFEST" hasher ".opencode/skills/hasher/SKILL.md")"
...
cat > "$FIX/.opencode/commands/c1.md" <<'CMD'
---
