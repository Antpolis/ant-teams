...
check('AC-T5-001: initMeta.version matches INIT_PROJECT_VERSION (0.2.0 at T2/T5 scope)', () => {
  const tmp = mkdtempRepo('ac001-ver');
  runInit(tmp);
  const cfg = readJson(path.join(tmp, '.github-project.json'));
  assert.ok(typeof cfg.initMeta.version === 'string' && cfg.initMeta.version.length > 0);
  assert.strictEqual(cfg.initMeta.version, '0.2.0');
});
...
// --- AC-T5-006: --force backup behavior (dormant at T5) ---------------------

process.stdout.write('Suite: AC-T5-006 --force backup (dormant at T5 scope)\n');

check('AC-T5-006 (dormant): --force accepted; AGENTS.md is not yet generated (T3 scope)', () => {
  // AGENTS.md generation ships with T3 / issue #4. At T5 scope, --force is
  // parsed (T2) but does not trigger AGENTS.md overwrite — so the .bak
  // helper has nothing to back up. This test pins that boundary so a
  // regression that generates AGENTS.md without the backup helper is
  // caught.
  const tmp = mkdtempRepo('ac006');
  // Pre-existing AGENTS.md that an operator may have written by hand.
  ...
  // AGENTS.md must NOT be modified at T5 scope (no T3 generation wired).
  const after = fs.readFileSync(path.join(tmp, 'AGENTS.md'), 'utf8');
  assert.strictEqual(after, 'hand-written agents.md', 'T5 must not touch AGENTS.md');
  // No backup should be created at T5 scope.
  const backups = fs.readdirSync(tmp).filter((n) => n.startsWith('AGENTS.md.bak'));
  assert.strictEqual(backups.length, 0, `expected no .bak files at T5 scope, got ${backups}`);
});
