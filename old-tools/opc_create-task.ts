...
export default tool({
  description: "Run scripts/create-task.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/create-task.sh`
    const fallback = `${import.meta.dir}/../../scripts/create-task.sh`
    ...
    if (code !== 0) throw new Error(out || "create-task failed")
    return out || "Completed create-task"
  },
})
