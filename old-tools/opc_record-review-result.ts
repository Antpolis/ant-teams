...
export default tool({
  description: "Run scripts/record-review-result.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/record-review-result.sh`
    const fallback = `${import.meta.dir}/../../scripts/record-review-result.sh`
    ...
    if (code !== 0) throw new Error(out || "record-review-result failed")
    return out || "Completed record-review-result"
  },
})
