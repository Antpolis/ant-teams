...
export default tool({
  description: "Run scripts/update-document-index.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/update-document-index.sh`
    const fallback = `${import.meta.dir}/../../scripts/update-document-index.sh`
    ...
    if (code !== 0) throw new Error(out || "update-document-index failed")
    return out || "Completed update-document-index"
  },
})
