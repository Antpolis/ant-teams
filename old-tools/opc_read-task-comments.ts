...
export default tool({
  description: "Run scripts/read-task-comments.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/read-task-comments.sh`
    const fallback = `${import.meta.dir}/../../scripts/read-task-comments.sh`
    ...
    if (code !== 0) throw new Error(out || "read-task-comments failed")
    return out || "Completed read-task-comments"
  },
})
