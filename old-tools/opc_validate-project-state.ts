...
export default tool({
  description: "Run scripts/validate-project-state.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/validate-project-state.sh`
    const fallback = `${import.meta.dir}/../../scripts/validate-project-state.sh`
    ...
    if (code !== 0) throw new Error(out || "validate-project-state failed")
    return out || "Completed validate-project-state"
  },
})
