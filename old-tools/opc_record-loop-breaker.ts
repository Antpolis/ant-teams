...
export default tool({
  description: "Run scripts/record-loop-breaker.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/record-loop-breaker.sh`
    const fallback = `${import.meta.dir}/../../scripts/record-loop-breaker.sh`
    ...
    if (code !== 0) throw new Error(out || "record-loop-breaker failed")
    return out || "Completed record-loop-breaker"
  },
})
