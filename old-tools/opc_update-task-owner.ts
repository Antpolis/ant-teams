...
export default tool({
  description: "Run scripts/update-task-owner.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/update-task-owner.sh`
    const fallback = `${import.meta.dir}/../../scripts/update-task-owner.sh`
    ...
    if (code !== 0) throw new Error(out || "update-task-owner failed")
    return out || "Completed update-task-owner"
  },
})
