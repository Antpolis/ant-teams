...
export default tool({
  description: "Run scripts/open-review-loop.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/open-review-loop.sh`
    const fallback = `${import.meta.dir}/../../scripts/open-review-loop.sh`
    ...
    if (code !== 0) throw new Error(out || "open-review-loop failed")
    return out || "Completed open-review-loop"
  },
})
