...
export default tool({
  description: "Run scripts/record-merge.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/record-merge.sh`
    const fallback = `${import.meta.dir}/../../scripts/record-merge.sh`
    ...
    if (code !== 0) throw new Error(out || "record-merge failed")
    return out || "Completed record-merge"
  },
})
