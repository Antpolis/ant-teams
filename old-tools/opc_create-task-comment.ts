...
export default tool({
  description: "Run scripts/create-task-comment.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/create-task-comment.sh`
    const fallback = `${import.meta.dir}/../../scripts/create-task-comment.sh`
    ...
    if (code !== 0) throw new Error(out || "create-task-comment failed")
    return out || "Completed create-task-comment"
  },
})
