...
export default tool({
  description: "Run scripts/record-pr.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/record-pr.sh`
    const fallback = `${import.meta.dir}/../../scripts/record-pr.sh`
    ...
    if (code !== 0) throw new Error(out || "record-pr failed")
    return out || "Completed record-pr"
  },
})
