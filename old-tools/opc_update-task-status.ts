...
export default tool({
  description: "Run scripts/update-task-status.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/update-task-status.sh`
    const fallback = `${import.meta.dir}/../../scripts/update-task-status.sh`
    ...
    if (code !== 0) throw new Error(out || "update-task-status failed")
    return out || "Completed update-task-status"
  },
})
