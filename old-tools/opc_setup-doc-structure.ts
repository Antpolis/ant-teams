...
export default tool({
  description: "Run scripts/setup-doc-structure.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/setup-doc-structure.sh`
    const fallback = `${import.meta.dir}/../../scripts/setup-doc-structure.sh`
    ...
    if (code !== 0) throw new Error(out || "setup-doc-structure failed")
    return out || "Completed setup-doc-structure"
  },
})
