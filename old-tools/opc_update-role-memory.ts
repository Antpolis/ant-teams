...
export default tool({
  description: "Run scripts/update-role-memory.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/update-role-memory.sh`
    const fallback = `${import.meta.dir}/../../scripts/update-role-memory.sh`
    ...
    if (code !== 0) throw new Error(out || "update-role-memory failed")
    return out || "Completed update-role-memory"
  },
})
