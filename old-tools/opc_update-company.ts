...
export default tool({
  description: "Run scripts/update-company.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/update-company.sh`
    const fallback = `${import.meta.dir}/../../scripts/update-company.sh`
    ...
    if (code !== 0) throw new Error(out || "update-company failed")
    return out || "Completed update-company"
  },
})
