...
export default tool({
  description: "Run scripts/resolve-blocker.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/resolve-blocker.sh`
    const fallback = `${import.meta.dir}/../../scripts/resolve-blocker.sh`
    ...
    if (code !== 0) throw new Error(out || "resolve-blocker failed")
    return out || "Completed resolve-blocker"
  },
})
