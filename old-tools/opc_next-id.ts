...
export default tool({
  description: "Run scripts/next-id.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/next-id.sh`
    const fallback = `${import.meta.dir}/../../scripts/next-id.sh`
    ...
    if (code !== 0) throw new Error(out || "next-id failed")
    return out || "Completed next-id"
  },
})
