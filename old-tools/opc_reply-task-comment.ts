...
export default tool({
  description: "Run scripts/reply-task-comment.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/reply-task-comment.sh`
    const fallback = `${import.meta.dir}/../../scripts/reply-task-comment.sh`
    ...
    if (code !== 0) throw new Error(out || "reply-task-comment failed")
    return out || "Completed reply-task-comment"
  },
})
