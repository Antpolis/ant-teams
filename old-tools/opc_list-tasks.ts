...
export default tool({
  description: "Run scripts/list-tasks.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/list-tasks.sh`
    const fallback = `${import.meta.dir}/../../scripts/list-tasks.sh`
    ...
    if (code !== 0) throw new Error(out || "list-tasks failed")
    return out || "Completed list-tasks"
  },
})
