...
export default tool({
  description: "Run scripts/record-pr-comment.sh",
  args: { args: tool.schema.array(tool.schema.string()).default([]), doc_root: tool.schema.string().default("docs") },
  async execute(args, context) {
    const local = `${context.worktree}/scripts/record-pr-comment.sh`
    const fallback = `${import.meta.dir}/../../scripts/record-pr-comment.sh`
    ...
    if (code !== 0) throw new Error(out || "record-pr-comment failed")
    return out || "Completed record-pr-comment"
  },
})
