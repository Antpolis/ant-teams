...
#
# Lists every script under $ANT_TEAM_SCRIPTS (populated by scripts/sync-company.sh,
# which installs the repository scripts/ tree to ~/.agents/scripts and exports
# ANT_TEAM_SCRIPTS in the shell rc files). Scripts without a recorded stable
# description are still listed with a generic note so new helpers stay visible
...
#      (scripts/sync-company.sh has not run in this environment)
#
set -euo pipefail
...
List the ant-team helper scripts installed by scripts/sync-company.sh into
$ANT_TEAM_SCRIPTS (~/.agents/scripts) with a one-line description each.

If ANT_TEAM_SCRIPTS is unset or missing, run scripts/sync-company.sh first.
USAGE
}

# Stable descriptions for the current helper scripts. Keys are the script
# basenames as installed by sync-company.sh. Keep one line per script and
# keep descriptions stable — agents and docs reference them.
describe_script() {
  case "$1" in
    ant-team-help.sh)
    ...
    create-task-branch.sh)
      ...
      ;;
    init-company.sh)
      printf 'Alias for sync-company.sh: install .opencode/ config, team scripts, and managed skills.'
      ;;
    init-project.sh)
      printf 'Repo-local wrapper for init-project-docs.sh: initialize a project repo for agentic delivery.'
      ;;
    init-project-docs.sh)
      printf 'Initialize or update a project repo: skills copy, .github-project.env seeding, AGENTS.md generation (engine lives with the project-initialization skill).'
      ;;
    record-communication.sh)
      ...
      ;;
    sync-company.sh)
      printf 'Install .opencode/ to ~/.config/opencode and team scripts to ~/.agents/scripts, then run the managed skills sync.'
      ;;
    sync-managed-skills.sh)
      printf 'Managed, non-destructive sync of repository-owned skills into ~/.agents/skills (manifest-tracked).'
      ;;
    update-company.sh)
      printf 'Alias for sync-company.sh: install .opencode/ config, team scripts, and managed skills.'
      ;;
...
if [[ -z "$scripts_dir" ]]; then
  ...
  echo "Run scripts/sync-company.sh first (installs ~/.agents/scripts and exports ANT_TEAM_SCRIPTS)." >&2
  exit 1
fi

if [[ ! -d "$scripts_dir" ]]; then
  echo "ANT_TEAM_SCRIPTS does not point to a directory: $scripts_dir" >&2
  echo "Run scripts/sync-company.sh again to reinstall the team helper scripts." >&2
  exit 1
fi
