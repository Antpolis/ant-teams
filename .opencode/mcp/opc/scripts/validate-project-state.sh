...
while IFS='|' read -r _ spec task title status owner branch pr loop blocker updated _rest; do
  ...
  if ! pm_task_exists "$spec" "$task"; then echo "Missing task file for $spec/$task" >&2; errors=$((errors+1)); fi
  if [[ ! "$loop" =~ ^[0-8]/8$ ]]; then echo "Invalid loop count for $spec/$task: $loop" >&2; errors=$((errors+1)); fi
  if [[ "$status" == "Done" && ! -f "$(pm_communication_log "$task")" ]]; then echo "Done task missing communication log: $spec/$task" >&2; errors=$((errors+1)); fi
done < "$board"
