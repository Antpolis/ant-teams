...
Prefer the `record-communication.sh` helper (installed at `$ANT_TEAM_SCRIPTS`, or `scripts/record-communication.sh` in the ant-teams repo) to record and list events — it fills the approved template and never writes to GitHub. If no template exists, stop and request one; do not invent the format.
