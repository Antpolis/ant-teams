...
| Loop Breaker | A custom agentic-flow decision point triggered when review loops reach the limit or a persistent architecture issue blocks progress. The tech-lead must inspect the task, spec, docs, guardrails, GitHub collaboration record, and architect memory to decide how to proceed. |
...
| Completed Task | A task whose definition of done, acceptance tests, verification, reviewer gate, GitHub collaboration record, and role-memory updates are all satisfied. |
