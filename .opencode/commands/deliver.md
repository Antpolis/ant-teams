...
8. After development, run validator review. Validator must read role memory and the GitHub collaboration record first. If review has findings, return to builder on the same task branch. Repeat development-review until validator clears the development, a hard blocker appears, or 8 loops are reached.
9. After every task or review loop, builder, validator, strategist, and tech-lead should review the GitHub collaboration record and update role memory using the role-memory skill when relevant.
